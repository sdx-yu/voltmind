...
export function VisualWorkspace({ project, nodes, entities, onBack, notify }: Props) {
  ...
  return <section className="visual-workspace">
    <header className="page-header"><div><button className="button ghost compact" onClick={onBack}><ArrowLeft size={15}/>返回交付</button><span className="eyebrow">视觉锚点 · 1.3.0</span><h2>让画面服从文字正典</h2><p>图片始终先成为候选；接受只绑定当前正典快照，不会用图片反写人物、地点、物品或剧情事实。</p></div><button className="button primary" onClick={() => setCreatingAnchor(true)}><Plus size={15}/>建立视觉锚点</button></header>
...
function StoryboardCardForm({ anchors, busy, onSubmit }: { anchors: VisualAnchor[]; busy: boolean; onSubmit: (input: { purpose: string; note: string; anchorIds: string[]; assetHash: string | null; visualDescription: string }) => void }) {
  ...
  return <form className="storyboard-form" onSubmit={(event) => { event.preventDefault(); onSubmit({ purpose, note, anchorIds: anchor ? [anchor.id] : [], assetHash: anchor?.acceptedAsset?.contentHash ?? null, visualDescription: anchor?.visualDescription ?? '' }); setPurpose(''); setNote('') }}><input aria-label="镜头目的" required maxLength={160} value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="镜头目的，如：建立空间关系"/><select aria-label="镜头视觉锚点" value={anchorId} onChange={(event) => setAnchorId(event.target.value)}><option value="">不绑定锚点</option>{anchors.map((item) => <option key={item.id} value={item.id}>{item.entityName}{item.bindingStatus === 'stale' ? '（待复核）' : ''}</option>)}</select><textarea aria-label="镜头备注" maxLength={2000} value={note} onChange={(event) => setNote(event.target.value)} placeholder="连续性或调度备注，不粘贴正文"/><button className="button primary full" disabled={busy || !purpose.trim()}><Plus size={14}/>添加分镜卡</button></form>
}
