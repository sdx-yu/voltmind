...
import { SelectControl, SelectField, TextareaField } from '../ui'
...
function AnchorForm({ entities, busy, onClose, onSubmit }: { entities: Entity[]; busy: boolean; onClose: () => void; onSubmit: (input: { entityId: string; selectedFields: VisualSelectedField[]; styleNote: string }) => void }) {
  ...
  return <Modal title="建立正典视觉锚点" onClose={onClose}><form className="form-stack" onSubmit={(event) => { event.preventDefault(); onSubmit({ entityId, selectedFields: fields, styleNote }) }}><p className="form-hint">只有下方明确勾选的字段会进入本地视觉描述。仅本地正典不可选。</p><SelectField label="正典项" required value={entityId} onValueChange={setEntityId}><option value="">选择人物、地点或物品</option>{eligible.map((item) => <option key={item.id} value={item.id} disabled={item.privacyLevel === 'local_private'}>{item.canonicalName}{item.privacyLevel === 'local_private' ? '（仅本地，不可读取）' : ''}</option>)}</SelectField><fieldset className="visual-field-picker"><legend>允许读取的字段</legend><label><input type="checkbox" checked disabled/>正典名称（必需）</label><label><input type="checkbox" checked={fields.includes('summary')} onChange={(event) => toggle('summary', event.target.checked)}/>简介{entity?.summary ? `：${entity.summary.slice(0, 50)}` : '（为空）'}</label><label><input type="checkbox" checked={fields.includes('aliases')} onChange={(event) => toggle('aliases', event.target.checked)}/>别名{entity?.aliases.length ? `：${entity.aliases.join('、')}` : '（为空）'}</label>{states.map((state) => <label key={state.id}><input type="checkbox" checked={fields.includes(`state:${state.id}`)} onChange={(event) => toggle(`state:${state.id}`, event.target.checked)}/>{state.attributeKey}：{String(state.value)}</label>)}</fieldset><TextareaField label="作者视觉要求" optional showCount maxLength={1000} value={styleNote} onChange={(event) => setStyleNote(event.target.value)} placeholder="例如：写实概念设定、冷灰色调；这是作者输入，不是文字正典"/><div className="modal-actions"><button type="button" className="button ghost" onClick={onClose}>取消</button><button className="button primary" disabled={busy || !entityId || entity?.privacyLevel === 'local_private'}>{busy ? '正在建立…' : '绑定当前正典'}</button></div></form></Modal>
}
