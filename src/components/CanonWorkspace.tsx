...
import { Archive, ArrowLeft, Box, Check, Clock3, Layers3, Link2, MapPin, Plus, ShieldCheck, Sparkles, Trash2, UserRound, X } from 'lucide-react'
...
import { Badge, Button, Card, IconButton, LibraryTemplate, ListRow, PageHeader, SearchField, SegmentedControl, SelectControl, TextareaField, TextField } from '../ui'
...
export function CanonWorkspace({ projectId, entities, nodes, refresh, onSelectScene, notify }: { projectId: string; entities: Entity[]; nodes: ManuscriptNode[]; refresh: () => Promise<void>; onSelectScene: (id: string) => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  return <LibraryTemplate className={`canon-workspace${detailVisible ? ' has-selection' : ''}`}>
    <aside className="ui-canon-pane" aria-label="正典导航"><header className="ui-canon-pane-header"><div><span className="ui-eyebrow">正典库</span><h1>故事中的事实</h1></div><Button variant="primary" size="small" leadingIcon={<Plus size={15} />} onClick={() => setCreating(true)}>新建</Button></header>
      ...
      <div className="ui-canon-list">{visible.map((entity) => <ListRow key={entity.id} selected={entity.id === selectedId && !candidateMode && !seriesMode} icon={<span className={`entity-icon entity-${entity.type}`}>{entityIcon(entity.type)}</span>} title={entity.canonicalName} description={entity.summary || typeLabel(entity.type)} trailing={entity.privacyLevel === 'local_private' ? <ShieldCheck size={13} aria-label="仅本地" /> : undefined} onClick={() => { setSelectedId(entity.id); setCandidateMode(false); setSeriesMode(false); setDetailOpen(true) }} />)}</div>
      <ListRow className="candidate-inbox series-entry" selected={seriesMode} icon={<Layers3 size={16} />} title="系列共享" meta="多部作品" onClick={() => { setSeriesMode(true); setCandidateMode(false); setDetailOpen(true) }} />
      <ListRow className={`candidate-inbox${candidates.length ? ' has-items' : ''}`} selected={candidateMode} icon={<Sparkles size={16} />} title="事实候选" meta={String(candidates.length)} onClick={() => { setCandidateMode(true); setSeriesMode(false); setDetailOpen(true) }} />
    </aside>
...
function EntityDetail({ entity, states, nodes, onRefresh, onDeleted, onSelectScene, notify }: { entity: Entity; states: EntityState[]; nodes: ManuscriptNode[]; onRefresh: () => Promise<void>; onDeleted: () => void; onSelectScene: (id: string) => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  return <div className="entity-detail-content">
    <PageHeader eyebrow={typeLabel(entity.type)} title={entity.canonicalName} description={entity.aliases.length ? `别名：${entity.aliases.join('、')}` : '暂无别名'} actions={<><Badge tone={entity.privacyLevel === 'local_private' ? 'success' : 'neutral'}>{privacyLabel}</Badge><IconButton onClick={() => setConfirmTrash(true)} label="移入回收站"><Trash2 size={16}/></IconButton></>} />
    <Card className="canon-card" title="简介与恒定事实" actions={<Button size="small" variant="secondary" onClick={() => void save()}>保存</Button>}><div className="ui-detail-form"><TextField label="别名" value={aliases} onChange={(event) => setAliases(event.target.value)} placeholder="用顿号分隔" /><TextareaField label="简介" rows={5} value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="只写不会随剧情频繁变化的身份、背景或规则。" /></div></Card>
    <Card className="canon-card" title="时态状态" description="描述它在不同故事时间点如何变化。" actions={<Button size="small" variant="primary" leadingIcon={<Plus size={14} />} onClick={() => setAdding(true)}>添加状态</Button>}>
...
function CreateEntityModal({ projectId, onClose, onCreated, notify }: { projectId: string; onClose: () => void; onCreated: (entity: Entity) => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  return <Modal title="新建正典项" onClose={onClose}><div className="form-stack"><label>类型<SelectControl value={type} onChange={(event) => setType(event.target.value as Entity['type'])}><option value="character">人物</option><option value="location">地点</option><option value="item">物品</option><option value="event">事件</option></SelectControl></label><label>名称<input autoFocus value={name} onChange={(event) => setName(event.target.value)} /></label><label>简介<textarea rows={3} value={summary} onChange={(event) => setSummary(event.target.value)} /></label><label>隐私<SelectControl value={privacy} onChange={(event) => setPrivacy(event.target.value as Entity['privacyLevel'])}><option value="normal">可用于 AI 上下文</option><option value="author_only">仅作者可见</option><option value="local_private">仅本地，不发送给云端 AI</option></SelectControl></label><div className="modal-actions"><button className="button ghost" onClick={onClose}>取消</button><button className="button primary" disabled={!name.trim()} onClick={() => void submit()}>创建</button></div></div></Modal>
}
