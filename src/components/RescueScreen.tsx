...
export function RescueScreen({ onRecovered, notify }: { onRecovered: () => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  return <main className="rescue-page"><section className="rescue-card">
    <div className="rescue-icon"><AlertTriangle size={30} /></div>
    <span className="eyebrow">安全救援模式</span>
    <h1>稿件库没有被继续写入</h1>
    <p>检测到主数据库异常，笔不怠已停止常规读写。你可以从校验通过的本地快照恢复；损坏原件不会被覆盖。</p>
    ...
    {confirming && <div className="rescue-confirm" role="alert"><strong>确认替换异常主库？</strong><p>将从 <code>{selected}</code> 恢复；当前损坏数据库及 WAL/SHM 会移入 recovery 目录，不会被删除。</p><div><button className="button ghost" disabled={busy} onClick={() => setConfirming(false)}>返回检查</button><button className="button primary" disabled={busy} onClick={() => void restore()}>{busy ? '正在校验并恢复…' : '确认恢复并重新打开'}</button></div></div>}
    {!confirming && <div className="rescue-actions"><button className="button secondary" onClick={() => window.location.reload()}><RefreshCcw size={16} />重新检测</button><button className="button primary" disabled={!selected} onClick={() => setConfirming(true)}><DatabaseBackup size={16} />恢复所选快照</button></div>}
