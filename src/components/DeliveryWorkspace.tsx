...
import { AlertTriangle, Check, CloudCog, Download, ExternalLink, FileArchive, FileCheck2, Fingerprint, Goal, LocateFixed, RefreshCw, ShieldAlert } from 'lucide-react'
...
type Props = {
  ...
  onOpenTool?: (tool: 'provenance' | 'sync') => void
...
export function DeliveryWorkspace({ project, nodes, onSelectScene, onOpenTool, notify }: Props) {
  ...
  return <section className="delivery-workspace">
    <header className="page-header"><div><span className="eyebrow">交付台</span><h2>把故事安全地带出去</h2><p>平台规则逐条可关闭；检查只提供有出处的提醒，不会阻止导出，也不会替你投稿。</p></div>{onOpenTool && <div className="page-header-tools"><button className="button ghost compact" onClick={() => onOpenTool('provenance')}><Fingerprint size={15} />创作来源</button><button className="button ghost compact" onClick={() => onOpenTool('sync')}><CloudCog size={15} />加密接力</button></div>}</header>
