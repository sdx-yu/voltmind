...
import type { CohortAttestation, CohortEvidenceClass, CohortSegment, ResearchBundleInspection, ResearchCohortStatus, ResearchWaveStatusResponse } from '../../shared/types'
import { api } from '../lib/api'
import { Button, PageHeader, SelectControl, WorkflowSteps, WorkflowTemplate } from '../ui'

type Props = { onBack: () => void; onOpenWaves: () => void; notify: (type: 'success' | 'error', message: string) => void }
...
export function ResearchCohortWorkspace({ onBack, onOpenWaves, notify }: Props) {
  const [status, setStatus] = useState<ResearchCohortStatus | null>(null)
  const [waveStatus, setWaveStatus] = useState<ResearchWaveStatusResponse | null>(null)
  ...
  async function refresh() { const [cohort, waves] = await Promise.all([api.getResearchCohortStatus(), api.getResearchWaveStatus()]); setStatus(cohort); setWaveStatus(waves) }
  ...
  async function importPackage() {
    ...
    setBusy(true)
    try {
      const result = await api.importResearchCohortPackage(staged.value, { evidenceClass, segment, attestation, retentionUntil: new Date(`${retentionDate}T23:59:59.999Z`).toISOString(), waveId: waveId || null })
  ...
  async function exportCohort() {
    setBusy(true)
    try { const bundle = await api.exportResearchCohort(); downloadJson(`笔不怠-R1B-${bundle.manifest.exportedAt.slice(0, 10)}.bbd-cohort`, bundle); notify('success', '聚合报告已导出；仍需人工复核，发布结论保持 NO-GO') }
  ...
  return <main className="cohort-workspace">
    <header className="workspace-topbar cohort-topbar"><div className="workspace-title"><button className="icon-button" onClick={onBack} aria-label="返回真实验证"><ArrowLeft size={18}/></button><span className="mini-brand">笔</span><div><strong>受控研究台</strong><small>仅限研究负责人 · 本机处理</small></div></div><span className="gate-badge no-go" aria-label="发布决策 NO-GO"><TriangleAlert size={14}/><span>发布决策 </span>NO-GO</span></header>
    <WorkflowTemplate className="cohort-content">
      <PageHeader eyebrow="只聚合证据，不认证身份" title="把真实样本与工程夹具彻底分开" description="导入在本机完成；只保留不可逆参与码哈希、负责人声明、事件哈希和有界摘要。" actions={<><Button variant="secondary" onClick={onOpenWaves}>波次执行</Button><Button variant="secondary" leadingIcon={<Download size={16}/>} disabled={busy} onClick={() => void exportCohort()}>导出聚合报告</Button></>} />
      <WorkflowSteps label="受控研究步骤" items={[{ id: 'inspect', label: '预检研究包', description: '验证完整性和事件链', state: staged ? 'complete' : 'current' }, { id: 'attest', label: '负责人核对', description: '区分夹具与真实样本', state: staged ? 'current' : 'upcoming' }, { id: 'aggregate', label: '聚合与复核', description: '数值不自动改变发布结论', state: status.participants.length ? 'current' : 'upcoming' }]} />

      <section className="cohort-gate-grid" aria-label="真实试点门">
      ...
      <div className="cohort-layout"><section className="panel-card cohort-import"><div className="section-title"><Upload size={20}/><div><h2>导入研究包</h2><p>默认按工程夹具处理；只有四项负责人声明齐全的外部包才可能计数。</p></div></div>
        ...
        <div className="form-stack cohort-form"><label>证据类型<SelectControl aria-label="证据类型" value={evidenceClass} onChange={(event) => { setEvidenceClass(event.target.value as CohortEvidenceClass); setWaveId('') }}><option value="engineering_fixture">工程夹具（不计数）</option><option value="external_attested">人工核对的真实外部样本</option></SelectControl></label><label>作者分层<SelectControl aria-label="作者分层" value={segment} onChange={(event) => setSegment(event.target.value as CohortSegment)}>{(Object.keys(segmentLabels) as CohortSegment[]).map((value) => <option key={value} value={value}>{segmentLabels[value]}</option>)}</SelectControl></label><label>保留至<input aria-label="保留至" type="date" min={tomorrowDate()} max={maxRetentionDate()} value={retentionDate} onChange={(event) => setRetentionDate(event.target.value)}/></label><label>执行波次<SelectControl aria-label="执行波次" value={waveId} onChange={(event) => setWaveId(event.target.value)}><option value="">不归属波次</option>{waveStatus?.waves.filter((wave) => ['recruiting', 'active', 'paused', 'review'].includes(wave.status) && (evidenceClass === 'external_attested' ? wave.kind === 'external_controlled' : wave.kind === 'engineering_rehearsal')).map((wave) => <option key={wave.id} value={wave.id}>{wave.displayCode} · {wave.kind === 'external_controlled' ? '真实受控' : '工程演练'}</option>)}</SelectControl></label></div>
