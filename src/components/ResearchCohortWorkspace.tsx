...
export function ResearchCohortWorkspace({ onBack, onOpenWaves, notify }: Props) {
  ...
  return <main className="cohort-workspace">
    <header className="workspace-topbar cohort-topbar"><div className="workspace-title"><button className="icon-button" onClick={onBack} aria-label="返回真实验证"><ArrowLeft size={18}/></button><span className="mini-brand">笔</span><div><strong>R1-B/C 受控研究台</strong><small>1.6.0 · 仅限本机研究负责人</small></div></div><span className="gate-badge no-go" aria-label="R1 决策 NO-GO"><TriangleAlert size={14}/><span>R1 决策 </span>NO-GO</span></header>
    <section className="cohort-content">
      <header className="page-header"><div><span className="eyebrow">只聚合证据，不认证身份</span><h1>把真实样本与工程夹具彻底分开</h1><p>导入在本机完成。数据库不保存原始研究包或参与码，只保存不可逆参与码哈希、负责人声明、事件哈希和有界摘要。</p></div><div className="inline-actions"><button className="button secondary" onClick={onOpenWaves}>R1-C 波次执行台</button><button className="button secondary" disabled={busy} onClick={() => void exportCohort()}><Download size={16}/>导出聚合报告</button></div></header>
