...
export function Workspace({ project, initialView, onProjectUpdated, onBack, notify }: { project: Project; initialView?: 'write' | 'template'; onProjectUpdated?: (project: Project) => void; onBack: () => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  return <div className={`workspace ${focusMode ? 'workspace-focus' : ''} ${singleLayout ? 'workspace-narrow' : ''}`} style={workspaceStyle}>
    ...
    {view === 'write' && <EditorTemplate
      ...
      details={showReadAloud && !compactLayout ? <ReadAloudPanel projectId={project.id} nodes={nodes} currentNodeId={selectedId} onClose={() => setReadAloudOpen(false)} onSelectScene={selectScene} notify={notify} /> : showInspector && selectedNode ? <Inspector projectId={project.id} node={selectedNode} entities={entities} contentVersion={sceneContentVersion} refreshEntities={refreshEntities} onUpdateNode={async (patch) => updateNode(selectedNode.id, patch)} onRefreshTree={refreshTree} onReloadScene={() => setEditorEpoch((value) => value + 1)} notify={notify} /> : undefined}
    ...
    {selectedNode && <Drawer className="ui-template-drawer" title="场景检查器" description={selectedNode.title} open={inspectorDrawer} onOpenChange={setInspectorDrawerOpen}><div className="ui-template-drawer-content"><Inspector projectId={project.id} node={selectedNode} entities={entities} contentVersion={sceneContentVersion} refreshEntities={refreshEntities} onUpdateNode={async (patch) => updateNode(selectedNode.id, patch)} onRefreshTree={refreshTree} onReloadScene={() => setEditorEpoch((value) => value + 1)} notify={notify} /></div></Drawer>}
