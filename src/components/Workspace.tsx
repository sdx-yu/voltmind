...
import { CommandPalette, Drawer, DropdownMenu, EditorTemplate, IconButton, SelectControl, type CommandItem } from '../ui'

type PrimaryMode = 'write' | 'plan' | 'canon' | 'revision' | 'deliver'

export function Workspace({ project, initialView, onProjectUpdated, onBack, notify }: { project: Project; initialView?: 'write' | 'plot' | 'template'; onProjectUpdated?: (project: Project) => void; onBack: () => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  async function restoreEntity(id: string) { await api.restoreEntity(id); setDeletedEntities((current) => current.filter((entity) => entity.id !== id)); await refreshEntities(); notify('success', '正典项及原有场景引用已恢复') }
  ...
  const paletteItems: CommandItem[] = [
    ...
    { id: 'settings', title: '设置', description: 'AI、文风、故事时间、目标与显示', section: '全局工具', shortcut: '⌘,', icon: <Settings size={17} />, onSelect: () => { setSettingsTab('ai'); setSettings(true) } },
  ]
  ...
  return <div className={`workspace ${focusMode ? 'workspace-focus' : ''} ${singleLayout ? 'workspace-narrow' : ''}`} style={workspaceStyle}>
      ...
      <div className="topbar-actions">
        ...
        <div className="topbar-action-group topbar-action-global">
          ...
          <DropdownMenu label="项目更多操作" trigger={<IconButton label="项目更多操作"><Ellipsis size={18} /></IconButton>} items={[
            ...
            { id: 'trash', label: '内容回收站', icon: <Trash2 size={15} />, onSelect: () => void openTrash() },
    ...
    {view === 'write' && <EditorTemplate
      ...
      details={showReadAloud && !compactLayout ? <ReadAloudPanel projectId={project.id} nodes={nodes} currentNodeId={selectedId} onClose={() => setReadAloudOpen(false)} onSelectScene={selectScene} notify={notify} /> : showInspector && selectedNode ? <Inspector projectId={project.id} node={selectedNode} entities={entities} contentVersion={sceneContentVersion} refreshEntities={refreshEntities} onUpdateNode={async (patch) => updateNode(selectedNode.id, patch)} onRefreshTree={refreshTree} onReloadScene={() => setEditorEpoch((value) => value + 1)} onOpenVoiceSettings={() => { setSettingsTab('voice'); setSettings(true) }} onOpenTimeSettings={() => { setSettingsTab('time'); setSettings(true) }} notify={notify} /> : undefined}
    ...
    {selectedNode && <Drawer className="ui-template-drawer" title="场景检查器" description={selectedNode.title} open={inspectorDrawer} onOpenChange={setInspectorDrawerOpen}><div className="ui-template-drawer-content"><Inspector projectId={project.id} node={selectedNode} entities={entities} contentVersion={sceneContentVersion} refreshEntities={refreshEntities} onUpdateNode={async (patch) => updateNode(selectedNode.id, patch)} onRefreshTree={refreshTree} onReloadScene={() => setEditorEpoch((value) => value + 1)} onOpenVoiceSettings={() => { setSettingsTab('voice'); setSettings(true) }} onOpenTimeSettings={() => { setSettingsTab('time'); setSettings(true) }} notify={notify} /></div></Drawer>}
    ...
    {view === 'plot' && <PlotWorkspace projectId={project.id} nodes={nodes} entities={entities} initialView={initialView === 'plot' ? 'blueprint' : undefined} onSelectScene={selectScene} notify={notify} />}
    ...
    {trashOpen && <Modal title="内容回收站" onClose={() => setTrashOpen(false)} wide><div className="trash-groups"><section><h3>书稿结构</h3>{deletedNodes.length ? <div className="trash-list">{deletedNodes.filter((node) => !node.parentId || !deletedNodes.some((parent) => parent.id === node.parentId)).map((node) => <div key={node.id}><span><strong>{node.title}</strong><small>{node.type}</small></span><SelectControl aria-label={`${node.title} 恢复位置`} value={restoreParents[node.id] ?? node.parentId ?? ''} onChange={(event) => setRestoreParents((current) => ({ ...current, [node.id]: event.target.value }))}>{restoreTargets(node, nodes).map((target) => <option key={target.id} value={target.id}>{target.title}</option>)}</SelectControl><button className="button secondary compact" onClick={() => void restoreNode(node)}><RotateCcw size={14} />恢复</button></div>)}</div> : <p className="muted">没有已删除的章节或场景。</p>}</section><section><h3>正典项</h3>{deletedEntities.length ? <div className="trash-list">{deletedEntities.map((entity) => <div key={entity.id}><span><strong>{entity.canonicalName}</strong><small>{entity.type}</small></span><button className="button secondary compact" onClick={() => void restoreEntity(entity.id)}><RotateCcw size={14} />恢复</button></div>)}</div> : <p className="muted">没有已删除的正典项。</p>}</section></div></Modal>}
