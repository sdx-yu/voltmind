...
describe('ResearchWaveWorkspace', () => {
  ...
  it('offers only valid next states and reports incidents with bounded codes', async () => {
    ...
    await user.selectOptions(screen.getByLabelText(`${wave.displayCode}异常码`), 'data_loss')
    await user.selectOptions(screen.getByLabelText(`${wave.displayCode}异常级别`), 'critical')
