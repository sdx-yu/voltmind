...
export function SprintWorkspace({ project, nodes, activeSceneId, compact = false, onOpenScene, onOpenDetails, onBack, notify }: Props) {
  ...
  return <section className="sprint-workspace">
    ...
    {active ? <section className={`sprint-active-card ${active.clockStatus !== 'ok' ? 'needs-review' : ''}`}>
      ...
      <div className="sprint-form-grid"><label>范围<select value={scope} onChange={(event) => setScope(event.target.value as 'scene' | 'project')}><option value="scene">一个场景</option><option value="project">整个项目</option></select></label>{scope === 'scene' && <label>场景<select value={sceneId} onChange={(event) => setSceneId(event.target.value)}>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.title}</option>)}</select></label>}<label>时长（分钟）<input type="number" min={10} max={120} value={durationMinutes} onChange={(event) => setDurationMinutes(Number(event.target.value))}/></label><label>净新增目标<input type="number" min={1} max={50000} value={goalWords} onChange={(event) => setGoalWords(Number(event.target.value))}/></label></div>
    ...
    <div className="sprint-columns">
      ...
      <section className="sprint-card"><header><h3>离线小组看板</h3><span>手动交换成果卡</span></header>
        <div className="board-create"><input aria-label="看板名称" value={boardName} onChange={(event) => setBoardName(event.target.value)}/><select aria-label="目标周期" value={boardPeriod} onChange={(event) => setBoardPeriod(event.target.value as 'day' | 'week')}><option value="day">今日</option><option value="week">本周</option></select><input aria-label="小组目标字数" type="number" min={1} value={boardGoal} onChange={(event) => setBoardGoal(Number(event.target.value))}/><button className="button secondary compact" onClick={() => void createBoard()}><Users size={14}/>新建</button></div>
        {boards.length > 0 && <label>当前看板<select value={selectedBoard?.id ?? ''} onChange={(event) => setSelectedBoardId(event.target.value)}>{boards.map((board) => <option key={board.id} value={board.id}>{board.name}</option>)}</select></label>}
