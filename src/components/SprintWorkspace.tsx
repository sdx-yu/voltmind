...
export function SprintWorkspace({ project, nodes, activeSceneId, compact = false, onOpenScene, onOpenDetails, onBack, notify }: Props) {
  ...
  return <section className="sprint-workspace"><div className="sprint-content">
    <header className="sprint-hero">
      <div className="sprint-hero-copy">
        <div className="sprint-hero-nav">{onBack && <button className="button ghost compact" onClick={onBack}><ArrowLeft size={14}/>返回写作</button>}<span className="eyebrow">安静冲刺 · 可信成果</span></div>
        <h2>专注在字句，成果由版本证明</h2>
        <p>用一次有边界的专注，换来一张可验证、可分享，又不泄露正文的成果卡。</p>
        <div className="sprint-trust-row"><span><TimerReset size={13}/>只统计已保存版本</span><span><Flag size={13}/>成果卡不含书名与正文</span></div>
      </div>
      <button className="sprint-reminder" onClick={() => void requestNotification(notify)}><Bell size={17}/><span><strong>本地结束提醒</strong><small>仅在这台设备触发</small></span></button>
    </header>
    {active ? <section className={`sprint-active-card ${active.clockStatus !== 'ok' ? 'needs-review' : ''}`}>
      ...
      <div className="sprint-start-intro"><span className="sprint-start-icon"><TimerReset size={25}/></span><span className="eyebrow">开始前设置</span><h3>开始一次可信冲刺</h3><p>开始与结束各冻结一次版本快照，只计算这段时间里真正保存下来的新增字数。</p><div className="sprint-start-notes"><span>10–120 分钟</span><span>不监控输入状态</span><span>随时可以暂停</span></div></div>
      <div className="sprint-start-config">
        <header><strong>本次冲刺</strong><span>设置范围、时间与目标</span></header>
        <div className="sprint-form-grid"><label>写作范围<SelectControl value={scope} onChange={(event) => setScope(event.target.value as 'scene' | 'project')}><option value="scene">一个场景</option><option value="project">整个项目</option></SelectControl></label>{scope === 'scene' && <label>目标场景<SelectControl value={sceneId} onChange={(event) => setSceneId(event.target.value)}>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.title}</option>)}</SelectControl></label>}<label>时长（分钟）<input type="number" min={10} max={120} value={durationMinutes} onChange={(event) => setDurationMinutes(Number(event.target.value))}/></label><label>净新增目标<input type="number" min={1} max={50000} value={goalWords} onChange={(event) => setGoalWords(Number(event.target.value))}/></label></div>
        <button className="button primary sprint-start-action" disabled={busy || (scope === 'scene' && !sceneId)} onClick={() => void start()}><Play size={15}/>开始并进入安静写作</button>
      </div>
    </section>}

    <div className="sprint-columns">
      <section className="sprint-card sprint-results-card"><header><div><span className="sprint-card-icon"><Flag size={17}/></span><span><h3>可验证成果卡</h3><small>你的本地冲刺记录</small></span></div><strong>{completed.length} 张</strong></header><div className="sprint-result-list">{completed.map((session) => { const card = session.resultCard!; return <article key={card.id}><div><strong>{signed(card.netWords)} 字</strong><span>{card.participantLabel} · {Math.round(card.activeDurationMs / 60_000)} 分钟</span><small>{new Date(card.endedAt).toLocaleString('zh-CN')} · {card.eventCount} 个链式事件</small></div><div><button className="button ghost compact" onClick={() => void exportCard(card.id)}><Download size={13}/>导出</button>{selectedBoard && <button className="button ghost compact" onClick={() => void addLocal(selectedBoard.id, card.id)}>加入看板</button>}</div></article>})}{!completed.length && <div className="sprint-empty-state"><span><Flag size={20}/></span><strong>还没有成果卡</strong><p>完成第一次冲刺后，可信净新增会在这里沉淀。</p><small>成果卡可导出或加入看板，但不会包含你的正文。</small></div>}</div></section>
      <section className="sprint-card sprint-board-card"><header><div><span className="sprint-card-icon"><Users size={17}/></span><span><h3>离线小组看板</h3><small>交换成果，不上传正文</small></span></div><strong>{boards.length} 个</strong></header>
        <section className="board-builder"><header><strong>新建一个目标看板</strong><span>数据只保存在本机</span></header><div className="board-create"><label className="board-name-field">看板名称<input aria-label="看板名称" value={boardName} onChange={(event) => setBoardName(event.target.value)}/></label><label>周期<SelectControl aria-label="目标周期" value={boardPeriod} onChange={(event) => setBoardPeriod(event.target.value as 'day' | 'week')}><option value="day">今日</option><option value="week">本周</option></SelectControl></label><label>小组目标字数<input aria-label="小组目标字数" type="number" min={1} value={boardGoal} onChange={(event) => setBoardGoal(Number(event.target.value))}/></label><div className="board-create-footer"><span>创建后即可添加本机或他人的成果卡</span><button className="button secondary compact" onClick={() => void createBoard()}><Users size={14}/>新建看板</button></div></div></section>
        {selectedBoard ? <>
          <section className="board-current"><header><strong>当前看板</strong>{boards.length > 0 && <SelectControl value={selectedBoard.id} onChange={(event) => setSelectedBoardId(event.target.value)}>{boards.map((board) => <option key={board.id} value={board.id}>{board.name}</option>)}</SelectControl>}</header><div className="board-summary"><div className="board-total"><CalendarDays size={20}/><span><strong>{selectedBoard.totalNetWords.toLocaleString('zh-CN')} / {selectedBoard.targetWords.toLocaleString('zh-CN')}</strong><small>{selectedBoard.period === 'day' ? '当日目标' : '本周目标'} · {selectedBoard.entries.length} 张成果卡</small></span></div><progress value={Math.max(0, selectedBoard.totalNetWords)} max={selectedBoard.targetWords}/><div className="board-participants">{selectedBoard.participants.map((person) => <span key={person.participantLabel}><strong>{person.participantLabel}</strong><em>{person.netWords} 字 · {person.sprintCount} 次</em></span>)}</div></div></section>
          <section className="board-import"><header><strong>导入他人的成果卡</strong><span>选择文件后先校验，再加入当前看板</span></header><label className="review-file"><Upload size={18}/><span>{packageName || '选择 .bbd-sprint 成果卡'}</span><input aria-label="选择冲刺成果卡" type="file" accept=".bbd-sprint,application/json" onChange={(event) => void readPackage(event.target.files?.[0])}/></label>{inspection && <div className="sprint-inspection"><strong>{inspection.participantLabel} · {signed(inspection.netWords)} 字</strong><span>{Math.round(inspection.activeDurationMs / 60_000)} 分钟 · 事件链 {inspection.eventCount} 项</span></div>}<button className="button primary full" disabled={!inspection} onClick={() => void importCard()}>校验后加入当前看板</button></section>
        </> : <div className="board-empty-state"><span><Users size={21}/></span><div><strong>先建立一个看板</strong><p>有了目标容器，才会开放成果卡导入与小组累计。</p></div></div>}
      </section>
    </div>
  </div></section>
}
