...
function CreateStyleSampleModal({ seriesId, projectId, onClose, onCreated, notify }: { seriesId: string; projectId: string; onClose: () => void; onCreated: () => Promise<void>; notify: Notice }) {
  ...
  return <Modal title="新增风格样本" onClose={onClose} wide><div className="form-stack"><label>作用范围<SelectControl aria-label="样本作用范围" value={scope} onChange={(event) => setScope(event.target.value as 'series' | 'project')}><option value="series">系列共享</option><option value="project">仅本书</option></SelectControl></label><label>样本标题<input aria-label="样本标题" autoFocus value={title} onChange={(event) => setTitle(event.target.value)}/></label><label>原文<textarea aria-label="样本原文" rows={8} value={content} onChange={(event) => setContent(event.target.value)} placeholder="只粘贴你有权使用的文本"/></label><label>使用指导<textarea aria-label="样本使用指导" value={guidance} onChange={(event) => setGuidance(event.target.value)} placeholder="例如：参考短句节奏，不复用专有名词"/></label><label>隐私<SelectControl value={privacyLevel} onChange={(event) => setPrivacy(event.target.value as PrivacyLevel)}><option value="normal">允许发送给已配置的 AI</option><option value="author_only">仅作者授权后发送</option><option value="local_private">仅本地，禁止发送</option></SelectControl></label><div className="modal-actions"><button className="button ghost" onClick={onClose}>取消</button><button className="button primary" disabled={!title.trim() || !content.trim()} onClick={() => void submit()}>保存并启用</button></div></div></Modal>
}
