...
import { SelectControl, SelectField, TextareaField, TextField } from '../ui'
...
function CreateSeriesModal({ projectId, onClose, onCreated, notify }: { projectId: string; onClose: () => void; onCreated: () => Promise<void>; notify: Notice }) {
  ...
  return <Modal title="创建系列" onClose={onClose}><div className="form-stack"><TextField label="系列名称" required autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：雾港纪事"/><TextareaField label="系列简介" optional value={description} onChange={(event) => setDescription(event.target.value)} placeholder="说明共同世界、时代或主题"/><div className="modal-actions"><button className="button ghost" onClick={onClose}>取消</button><button className="button primary" disabled={!name.trim()} onClick={() => void submit()}>创建并加入本书</button></div></div></Modal>
}
...
function CreateCanonModal({ seriesId, projectId, onClose, onCreated, notify }: { seriesId: string; projectId: string; onClose: () => void; onCreated: () => Promise<void>; notify: Notice }) {
  ...
  return <Modal title="新增系列共享项" onClose={onClose}><div className="form-stack"><SelectField label="类型" value={type} onValueChange={(value) => setType(value as EntityType)}><option value="character">人物</option><option value="location">地点</option><option value="item">物品</option><option value="event">事件</option></SelectField><TextField label="名称" required autoFocus value={name} onChange={(event) => setName(event.target.value)}/><TextField label="别名" optional value={aliases} onChange={(event) => setAliases(event.target.value)} placeholder="用顿号分隔"/><TextareaField label="系列基线" optional value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="所有作品共同遵守的事实"/><div className="modal-actions"><button className="button ghost" onClick={onClose}>取消</button><button className="button primary" disabled={!name.trim()} onClick={() => void submit()}>保存共享基线</button></div></div></Modal>
}
