...
import { Badge, Button, Card, IconButton, SegmentedControl, SelectControl, TextareaField, TextField } from '../ui'
...
function AddProfileFieldModal({ entity, onClose, onCreated, notify }: { entity: Entity; onClose: () => void; onCreated: () => void; notify: Notify }) {
  ...
  return <Modal title={`添加 ${entity.canonicalName} 的档案字段`} onClose={onClose}><div className="form-stack"><label>分类<input list="profile-categories" value={category} onChange={(event) => setCategory(event.target.value)}/><datalist id="profile-categories"><option value="身份"/><option value="外貌"/><option value="背景"/><option value="性格"/><option value="语言"/><option value="目标"/><option value="其他"/></datalist></label><label>字段名<input autoFocus value={label} onChange={(event) => setLabel(event.target.value)} placeholder="如：公开身份 / 口头禅"/></label><label>内容<textarea rows={4} value={value} onChange={(event) => setValue(event.target.value)}/></label><label>隐私<SelectControl value={privacy} onChange={(event) => setPrivacy(event.target.value as PrivacyLevel)}><option value="normal">可用于 AI 上下文</option><option value="author_only">仅作者</option><option value="local_private">仅本地</option></SelectControl></label><div className="modal-actions"><Button variant="ghost" onClick={onClose}>取消</Button><Button variant="primary" disabled={!category.trim() || !label.trim()} onClick={() => void submit()}>添加</Button></div></div></Modal>
}
...
function AddRelationshipModal({ projectId, entity, entities, onClose, onCreated, notify }: { projectId: string; entity: Entity; entities: Entity[]; onClose: () => void; onCreated: () => void; notify: Notify }) {
  ...
  return <Modal title={`为 ${entity.canonicalName} 建立关系`} onClose={onClose}><div className="form-stack"><label>关联正典项<SelectControl value={target} onChange={(event) => setTarget(event.target.value)}>{options.map((item) => <option key={item.id} value={item.id}>{item.canonicalName} · {entityTypeLabel(item.type)}</option>)}</SelectControl></label><label>关系类型<SelectControl value={type} onChange={(event) => setType(event.target.value)}>{Object.entries(RELATION_LABELS).map(([value, text]) => <option key={value} value={value}>{text}</option>)}</SelectControl></label><label>方向<SelectControl value={direction} onChange={(event) => setDirection(event.target.value as EntityRelationship['direction'])}><option value="mutual">双向关系</option><option value="directed">从当前项指向关联项</option></SelectControl></label><TextField label="显示名称（可选）" value={label} onChange={(event) => setLabel(event.target.value)} placeholder="如：表面盟友 / 监护人"/><TextareaField label="关系说明" rows={3} value={summary} onChange={(event) => setSummary(event.target.value)}/><label>隐私<SelectControl value={privacy} onChange={(event) => setPrivacy(event.target.value as PrivacyLevel)}><option value="normal">可用于 AI 上下文</option><option value="author_only">仅作者</option><option value="local_private">仅本地</option></SelectControl></label><div className="modal-actions"><Button variant="ghost" onClick={onClose}>取消</Button><Button variant="primary" disabled={!target || !type.trim()} onClick={() => void submit()}>建立关系</Button></div></div></Modal>
}
...
function AddRelationshipStateModal({ relationship, nodes, onClose, onCreated, notify }: { relationship: EntityRelationship; nodes: ManuscriptNode[]; onClose: () => void; onCreated: () => void; notify: Notify }) {
  ...
  return <Modal title="添加关系变化" onClose={onClose}><div className="form-stack"><TextField label="状态" value={status} onChange={(event) => setStatus(event.target.value)} placeholder="如：互相信任 / 暂时决裂"/><TextareaField label="说明" rows={3} value={note} onChange={(event) => setNote(event.target.value)}/><label>从场景起生效<SelectControl value={from} onChange={(event) => setFrom(event.target.value)}><option value="">未指定</option>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.title}</option>)}</SelectControl></label><label>到场景前失效<SelectControl value={to} onChange={(event) => setTo(event.target.value)}><option value="">至今</option>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.title}</option>)}</SelectControl></label><TextField label="故事时间起点" value={worldFrom} onChange={(event) => setWorldFrom(event.target.value)} placeholder="可选，如 2035-04-18"/><TextField label="故事时间终点" value={worldTo} onChange={(event) => setWorldTo(event.target.value)} placeholder="可选，区间不含终点"/><TextareaField label="正文证据" rows={2} value={evidence} onChange={(event) => setEvidence(event.target.value)} placeholder="粘贴能证明这次变化的原句"/><div className="modal-actions"><Button variant="ghost" onClick={onClose}>取消</Button><Button variant="primary" disabled={!status.trim()} onClick={() => void submit()}>加入时间线</Button></div></div></Modal>
}
