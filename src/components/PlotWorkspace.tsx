...
import { ArrowRight, CheckCircle2, Clock3, GitBranch, Layers3, Plus, Sparkles, Trash2 } from 'lucide-react'
...
export function PlotWorkspace({ projectId, nodes, entities, onSelectScene, notify }: { projectId: string; nodes: ManuscriptNode[]; entities: Entity[]; onSelectScene: (id: string) => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  return <section className="plot-workspace"><header className="page-header"><div><span className="eyebrow">剧情台 · V1-A</span><h2>从叙述顺序看见故事时间</h2><p>同一份场景数据驱动双时间轴；伏笔的每次变化都保留场景证据。</p></div><div className="segmented plot-tabs"><button className={view === 'narrative' ? 'active' : ''} onClick={() => setView('narrative')}><Layers3 size={15} />叙述顺序</button><button className={view === 'story' ? 'active' : ''} onClick={() => setView('story')}><Clock3 size={15} />故事时间</button><button className={view === 'foreshadows' ? 'active' : ''} onClick={() => setView('foreshadows')}><GitBranch size={15} />伏笔看板</button></div></header>
    ...
    {view === 'foreshadows' && <div className="foreshadow-workspace"><div className="foreshadow-toolbar"><div><strong>{foreshadows.filter((item) => item.status !== 'resolved').length} 条未回收</strong><span>高优先级 {foreshadows.filter((item) => item.status !== 'resolved' && item.importance === 'high').length} 条</span></div><button className="button primary" onClick={() => setCreating(true)}><Plus size={16} />建立伏笔</button></div><div className="foreshadow-board">{stages.map((stage) => <section className={`foreshadow-column stage-${stage.status}`} key={stage.status}><header><div><strong>{stage.label}</strong><span>{foreshadows.filter((item) => item.status === stage.status).length}</span></div><small>{stage.hint}</small></header><div>{foreshadows.filter((item) => item.status === stage.status).map((item) => <article className="foreshadow-card" key={item.id}><div className="foreshadow-card-head"><span className={`importance importance-${item.importance}`}>{importanceLabel(item.importance)}</span><button className="icon-button" onClick={() => setPendingTrash(item)} aria-label={`移除伏笔 ${item.title}`}><Trash2 size={13}/></button></div><h3>{item.title}</h3>{item.summary && <p>{item.summary}</p>}{item.plannedPayoff && <div className="payoff"><Sparkles size={13}/><span>计划回收：{item.plannedPayoff}</span></div>}<ol className="foreshadow-history">{item.events.slice(-3).map((event) => <li key={event.id}><span>{stageLabel(event.action)}</span><small>{sceneTitle(event.nodeId, scenes)}{event.evidence ? ` · “${event.evidence}”` : ''}</small></li>)}</ol><div className="foreshadow-actions">{nextActions(item.status).map((action) => <button key={action} onClick={() => setTransitioning({ item, action })}>{action === 'resolved' ? <CheckCircle2 size={13}/> : <ArrowRight size={13}/>} {stageLabel(action)}</button>)}</div></article>)}</div></section>)}</div></div>}
...
function SceneCard({ scene, entities, onSelect }: { scene: ManuscriptNode; entities: Entity[]; onSelect: (id: string) => void }) { return <button className="scene-card" onClick={() => onSelect(scene.id)}><span className={`status-line status-${scene.status}`} /><strong>{scene.title}</strong><p>{scene.povEntityId ? `POV · ${entities.find((entity) => entity.id === scene.povEntityId)?.canonicalName ?? '未知'}` : '未指定 POV'} · {sceneStatusLabel(scene.status)}</p><footer><span>{scene.wordCount} 字</span><span>{scene.storyTime || '时间未定'}</span></footer></button> }
