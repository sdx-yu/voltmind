...
import { BoardTemplate, Button, IconButton, PageHeader, SegmentedControl, SelectableCard, SelectControl } from '../ui'
...
export function PlotWorkspace({ projectId, nodes, entities, onSelectScene, notify }: { projectId: string; nodes: ManuscriptNode[]; entities: Entity[]; onSelectScene: (id: string) => void; notify: (type: 'success' | 'error', message: string) => void }) {
  ...
  return <BoardTemplate className="plot-workspace"><PageHeader tone="editorial" eyebrow="规划" title="从叙述顺序看见故事时间" description="同一份场景数据驱动双时间轴；伏笔的每次变化都保留场景证据。" actions={<SegmentedControl label="规划视图" value={view} onChange={(value) => setView(value as PlotView)} items={[{ id: 'narrative', label: '叙述顺序' }, { id: 'story', label: '故事时间' }, { id: 'foreshadows', label: '伏笔看板' }]} />} />
...
function TransitionForm({ item, action, scenes, busy, onClose, onSubmit }: { item: Foreshadow; action: ForeshadowStatus; scenes: ManuscriptNode[]; busy: boolean; onClose: () => void; onSubmit: (input: TransitionInput) => void }) {
  ...
  return <Modal title={`${stageLabel(action)} · ${item.title}`} onClose={onClose}><form className="form-stack" onSubmit={(event) => { event.preventDefault(); onSubmit({ nodeId: nodeId || null, evidence, note }) }}><p className="form-hint">本次变化会追加到证据链，不会覆盖之前的阶段记录。</p><label>发生场景<SelectControl value={nodeId} onChange={(event) => setNodeId(event.target.value)}><option value="">暂不关联</option>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.title}</option>)}</SelectControl></label><label>正文证据<input value={evidence} onChange={(event) => setEvidence(event.target.value)} placeholder="例如：他第三次拨动停摆的指针" /></label><label>作者备注<textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="记录这次安排的意图" /></label><div className="modal-actions"><Button variant="ghost" onClick={onClose}>取消</Button><Button type="submit" variant="primary" loading={busy}>{busy ? '正在保存…' : `确认${stageLabel(action)}`}</Button></div></form></Modal>
}
