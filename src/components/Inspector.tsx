...
function ScenePanel({ node, entities, onUpdateNode, onRefreshTree, onReloadScene, notify }: Pick<Props, 'node' | 'entities' | 'onUpdateNode' | 'onRefreshTree' | 'onReloadScene' | 'notify'>) {
  ...
  return <div className="panel-stack">
    <section className="inspector-section"><header><span className="section-icon"><MessageSquareText size={15} /></span><h3>场景状态</h3></header>
      <label className="field-label">进度<select value={node.status} onChange={(event) => void update({ status: event.target.value as ManuscriptNode['status'] })}>
      ...
      <label className="field-label">故事时间<input value={node.storyTime ?? ''} onChange={(event) => void update({ storyTime: event.target.value || null })} placeholder="建议 YYYY-MM-DD，便于顺序检查" /></label>
      <label className="field-label">视角人物<select value={node.povEntityId ?? ''} onChange={(event) => void update({ povEntityId: event.target.value || null })}><option value="">未指定</option>{entities.filter((entity) => entity.type === 'character').map((entity) => <option key={entity.id} value={entity.id}>{entity.canonicalName}</option>)}</select></label>
...
function CanonPanel({ projectId, node, entities, refreshEntities, notify }: Pick<Props, 'projectId' | 'node' | 'entities' | 'refreshEntities' | 'notify'>) {
  ...
  return <div className="panel-stack">
    ...
    <section className="inspector-section"><header><span className="section-icon"><Sparkles size={15} /></span><h3>快速加入正典</h3></header>
      <div className="inline-form"><select value={type} onChange={(event) => setType(event.target.value as Entity['type'])}><option value="character">人物</option><option value="location">地点</option><option value="item">物品</option><option value="event">事件</option></select><input value={name} onChange={(event) => setName(event.target.value)} placeholder="名称" /><button className="button secondary" disabled={!name.trim()} onClick={() => void addEntity()}>加入</button></div>
    </section>
...
function GrantKnowledgeModal({ fact, entity, nodes, onClose, onSaved, notify }: { fact: KnowledgeFact; entity: Entity; nodes: ManuscriptNode[]; onClose: () => void; onSaved: () => void; notify: Props['notify'] }) {
  ...
  return <Modal title={`${entity.canonicalName}何时知道“${fact.title}”`} onClose={onClose}><div className="form-stack"><label>从场景起知情<select value={from} onChange={(event) => setFrom(event.target.value)}><option value="">请选择</option>{sceneNodes(nodes).map((scene) => <option key={scene.id} value={scene.id}>{scene.title}</option>)}</select></label><label>知情证据<input value={evidence} onChange={(event) => setEvidence(event.target.value)} placeholder="例如：他亲眼看见密信落款"/></label><label>作者备注<textarea value={note} onChange={(event) => setNote(event.target.value)} /></label><div className="modal-actions"><button className="button ghost" onClick={onClose}>取消</button><button className="button primary" disabled={!from} onClick={() => void submit()}>保存知情起点</button></div></div></Modal>
}
...
function AiPanel({ projectId, node, notify }: Pick<Props, 'projectId' | 'node' | 'notify'>) {
  ...
  useEffect(() => { void api.getContext(projectId, node.id).then(setContext).catch((error) => notify('error', error instanceof Error ? error.message : '上下文加载失败')); setResult(null); setAccepted(false) }, [projectId, node.id])
  ...
  return <div className="panel-stack">
    ...
    <section className="inspector-section"><header><span className="section-icon"><Bot size={15} /></span><h3>创作助手</h3></header>
      ...
      <button className="button primary full" disabled={running || selected.length === 0} onClick={() => void run()}><WandSparkles size={16} />{running ? '正在思考…' : '生成候选'}</button>
    </section>
