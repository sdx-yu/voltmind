...
export function TemplateWorkspace({ project, onBack, onChanged, notify }: Props) {
  ...
  return <section className="template-workspace">
    <header className="page-header template-header"><div className="template-header-copy"><Button className="template-header-back" size="small" variant="ghost" leadingIcon={<ArrowLeft size={15}/>} onClick={onBack}>返回规划</Button><div className="template-heading"><span className="eyebrow">本地结构目录</span><h2>先预览，再把结构放进书稿</h2><p>这里只安装声明式结构数据，不执行第三方代码；作者标签和来源均为自述，不是官方认证。</p></div></div><label className="button primary template-import"><Upload size={15}/>导入本地包<input aria-label="选择结构模板包" type="file" accept="application/json,.json,.bbd-template" onChange={(event) => { const file = event.target.files?.[0]; if (file) void inspectFile(file); event.currentTarget.value = '' }}/></label></header>
