...
export function SyncWorkspace({ project, onSynced, onBack, notify }: Props) {
  ...
  return <section className="sync-workspace">
    ...
    {pendingImport && <Modal title="接力包预检" onClose={() => setPendingImport(null)}><div className="sync-preview"><ShieldCheck size={23}/><h3>{pendingImport.inspection.projectTitle}</h3><p>{pendingImport.fileName} 已通过分块哈希、认证标签和恢复短语校验。</p><dl><div><dt>发送设备</dt><dd>{pendingImport.inspection.senderDeviceName}</dd></div><div><dt>序号</dt><dd>{pendingImport.inspection.sequence}</dd></div><div><dt>场景 / 正典</dt><dd>{pendingImport.inspection.sceneCount} / {pendingImport.inspection.entityCount}</dd></div><div><dt>来源事件</dt><dd>{pendingImport.inspection.provenanceEventCount}</dd></div></dl><button className="button primary" disabled={busy} onClick={() => void applyImport()}>{busy ? '正在合并…' : '确认应用到当前项目'}</button></div></Modal>}
  </section>
}
