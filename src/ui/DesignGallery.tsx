  ...
  return <main className="ui-gallery">
    ...
    <div className="ui-gallery-main">
      ...
      <GallerySection title="字段与选择" description="标签、帮助、错误、禁用和触控状态共用同一结构。">
        <div className="ui-gallery-grid">
          <Sample title="表单字段"><div className="ui-gallery-stack"><SearchField label="搜索故事事实" value={fieldSearch} onValueChange={setFieldSearch} placeholder="名称、别名或简介" /><TextField label="故事名称" required showCount maxLength={200} defaultValue="雾港来信" description="仅保存在本地项目中" /><SelectField label="场景状态" defaultValue="draft"><option value="idea">想法</option><option value="draft">草稿</option><option value="revising">修订中</option><option value="complete">完成</option><option value="published">已发布</option></SelectField><TextareaField label="场景目标" optional defaultValue="让主角在错误线索中发现真正的时间矛盾。" /><TextField label="章节编号" defaultValue="-1" error="章节编号必须大于 0" /></div></Sample>
