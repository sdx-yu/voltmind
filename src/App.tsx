...
export default function App() {
  ...
  return <><Toast toast={toast} onClose={() => setToast(null)} />{pwa.updateReady && <div className="pwa-update" role="status"><span>新版本已准备好</span><button onClick={() => activatePwaUpdate(pwa.registration)}>立即更新</button></div>}{mobile ? <MobileHome/> : rescueMode ? <RescueScreen onRecovered={() => { setRescueMode(false); setLoading(true); window.setTimeout(() => window.location.reload(), 300) }} notify={notify} /> : selected ? <Workspace project={selected} onBack={() => { setSelected(null); void refresh() }} notify={notify} /> : reviewInbox ? <ReviewWorkspace project={null} nodes={[]} reviewerOnly onSelectScene={() => undefined} onChanged={async () => undefined} onBack={() => setReviewInbox(false)} notify={notify}/> : <Bookshelf projects={projects} loading={loading} onOpen={setSelected} onRefresh={refresh} onOpenReview={() => setReviewInbox(true)} notify={notify} />}</>
}
