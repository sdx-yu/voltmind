...
| 能力 | 状态 | 实现位置 | 自动化/验收证据 |
|---|---|---|---|
| 固定知情同意 | 通过 | `r1-consent-v1`、`ResearchService.enroll`、`ResearchWorkspace` | 四项逐一确认、固定文本哈希、服务端复核；缺项拒绝，真实浏览器禁用/启用闭环 |
| 匿名任务证据 | 通过 | migration v14、`research_enrollments/tasks/events` | 不采集姓名/邮箱；五类任务、有界难度/分钟、固定问题码和不可逆项目范围哈希 |
| 可校验研究包 | 通过 | `server/research.ts`、`bbd-research-v1` | 严格 Schema、事件前向哈希、链头、整包 SHA-256；未知/禁用字段与篡改阻断 |
| 最小披露支持包 | 通过 | `buildSupportBundle`、`bbd-support-v1` | 只含版本、运行时大版本、健康、schema、快照/对象数量级；无标题、正文、ID、密钥或路径 |
| 退出与快照边界 | 通过 | 退出 API、级联外键、隐私/备份说明 | 清除当前数据库参与、任务、事件并保留书稿；明确旧 SQLite 快照需另行管理 |
| 项目备份隔离 | 通过 | `bbd-backup-v2` 项目导出 | 普通项目备份不含全局研究身份、参与码或研究事件；自动化往返测试 |
| 可见发布门 | 通过 | `ReleaseReadiness`、`check-release-readiness.mjs`、R1 UI | 工程/外部证据分栏；版本、签名结构和 DMG 实查；390px 持续显示 NO-GO |
| 桌面与既有能力回归 | 通过 | 1.4.0 `.app`/DMG、全量测试 | 32 文件 116 项测试、2,153 模块生产构建、真实浏览器、严格签名结构、JIT entitlement、DMG 与 sidecar 健康冒烟 |
| 真实种子作者 | 外部验证 | R1-B 受控研究 | 至少 5 名两周、3 名四周；本机夹具、自动化、单人自测和单纯导出均不计入 |
| 公开跨平台发布 | 外部验证 | 发布门 | 缺 Developer ID/Apple 公证、Windows x64 签名包、真实输入法和移动设备矩阵，公开发布保持 NO-GO |

## R1-B 受控种子研究台追踪

| 能力 | 状态 | 实现位置 | 自动化/验收证据 |
|---|---|---|---|
| v15 数据迁移 | 通过 | `research_cohort_participants/submissions/deletion_receipts` | v14→v15 迁移前完整备份；旧阶段迁移回归全部直达 v15 |
| 研究包严格预检 | 通过 | `parseResearchBundle`、`validateBundleSemantics` | Schema、manifest、事件链、同意、任务事件和声明进度一致；篡改、自由字段和语义漂移阻断 |
| 夹具/真实证据隔离 | 通过 | `evidence_class`、四项 `CohortAttestation` | 夹具独立计数且永不进入周门；真实外部首次导入缺任一声明即拒绝 |
| 连续提交与防分叉 | 通过 | 事件哈希前缀、同意回执、唯一链头 | 相同包幂等；严格扩展更新；回退、分叉和同意变化拒绝；首次证据类型/分层不可静默更换 |
| 固定周期门 | 通过 | UTC 周桶、跨度与 `aggregate` | 两周 2 桶+8 天，四周 4 桶+22 天，核心闭环 2 次；5/3/3/0 试点门有边界自动化 |
| 最小披露聚合 | 通过 | `bbd-cohort-v1` | 不含参与码、原始包、书名、正文、项目/任务 ID、Prompt、密钥或路径；manifest SHA-256 复算通过 |
| 删除与到期清除 | 通过 | 级联外键、双哈希 deletion receipt | 全部提交摘要级联删除；删除后阻止误重收；1–365 天保留边界服务端复核 |
| 可见负责人工作流 | 通过 | `ResearchCohortWorkspace`、响应式 CSS | 真实浏览器完成预检/夹具导入；真实门仍为 0；四项声明启用门；390px 无横向溢出、NO-GO 可见、控制台 0 告警 |
| 本机工程回归 | 通过 | 1.5.0 自动化与发行包 | 34 文件 124 项测试通过；桌面构建、签名结构、DMG、sidecar 冒烟见 1.5.0 验收报告 |
| 真实作者与研究结论 | 外部验证 | R1-C 受控波次 | 当前真实样本 0；需研究负责人核对外部招募/同意/原始包，自动化和夹具不计数 |
| 公开跨平台发布 | 外部验证 | R1 发布门 | 仍缺 Developer ID/公证、Windows 签名真机、移动真机与分发证据，R1 固定 NO-GO |

## R1-C 受控两周波次追踪

| 能力 | 状态 | 实现位置 | 自动化/验收证据 |
|---|---|---|---|
| v16 数据迁移 | 通过 | `research_waves/events/incidents`、参与者 `wave_id` | v15→v16 迁移前完整备份；所有历史迁移回归直达 v16 |
| 冻结协议与配额 | 通过 | `ResearchWaveService.createWave`、`r1c-wave-v1` | 14–42 天、真实 5–10 人、四分层总和精确匹配；创建后无修改端点 |
| 准备门与状态机 | 通过 | readiness、`transition` | 四项全真才可招募；跳步、窗口外执行、窗口结束前复核、复核回滚均阻断 |
| 包归属与原子性 | 通过 | `validateAssignment`、cohort import transaction | 真实/夹具类型隔离、同意窗口、总目标和分层配额；不可换波次；失败增量不残留绑定 |
| 有界异常与审计 | 通过 | fixed incident code/severity、wave events | 无自由文本；严重异常阻断关闭；事件顺序、前向哈希与整链复算通过 |
| 删除隐私边界 | 通过 | `linkCommitment`、级联参与者删除 | 事件仅存波次域二次哈希承诺，不复制参与者哈希；无名册执行包不含个人/书稿信息 |
| 可见执行工作流 | 通过 | `ResearchWaveWorkspace`、响应式 CSS | 真实浏览器完成演练创建、招募、执行、严重异常记录/解决；真实计数始终 0；390px 无溢出、console 0 告警 |
...
| 能力 | 状态 | 实现位置 | 自动化/验收证据 |
|---|---|---|---|
| v17 数据迁移 | 通过 | `entity_profile_fields`、`entity_relationships`、`relationship_states` | 升级前完整 SQLite 快照；全部历史迁移回归直达 v17 |
| 可自定义人物档案 | 通过 | `CanonProfilePanel`、档案字段 API | 分类/字段/长文本/隐私/排序数据模型；创建、修改、删除与重开持久化测试 |
...
| 加密接力 | 本机工程通过 | `bbd-sync-v1` 正典实体投影 | 档案和源实体拥有的关系进入实体内容哈希与结构化冲突；空库恢复测试通过 |
| 非范围能力 | 外部/后续验证 | 产品范围边界 | 组织新类型、家谱推导、自由白板、在线协作与云账号未实现，不在 UI 中伪装入口 |
