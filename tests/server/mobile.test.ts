...
describe('V1-M mobile companion contract', () => {
  ...
  it('backs up v8 and applies the v9–v11 immutable schemas', () => {
    ...
    result.database.db.exec('DROP TABLE template_events; DROP TABLE template_applications; DROP TABLE template_grants; DROP TABLE template_package_resources; DROP TABLE template_packages; DROP TABLE template_resources; DROP TABLE sprint_board_cards; DROP TABLE sprint_boards; DROP TABLE sprint_result_cards; DROP TABLE sprint_events; DROP TABLE sprint_samples; DROP TABLE sprint_sessions; DROP TABLE review_decisions; DROP TABLE review_feedback; DROP TABLE review_sessions; DROP TABLE mobile_inbox_actions; DROP TABLE mobile_inbox_items; DELETE FROM schema_migrations WHERE version IN (9,10,11,12);')
    result.database.close(); result = { ...result, database: new AppDatabase(databasePath) }
    expect(result.database.db.prepare('SELECT MAX(version) AS version FROM schema_migrations').get()).toMatchObject({ version: 12 })
    ...
    expect(fs.readdirSync(path.join(dir, 'backups')).some((name) => name.startsWith('pre-migration-v8-to-v12-'))).toBe(true)
  })
