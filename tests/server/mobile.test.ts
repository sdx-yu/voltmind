...
describe('V1-M mobile companion contract', () => {
  ...
  it('backs up v8 and applies the v9–v11 immutable schemas', () => {
    ...
    result.database.db.exec('DROP TABLE story_beat_scenes; DROP TABLE story_beats; DROP TABLE story_blueprints; DELETE FROM schema_migrations WHERE version=21;')
    ...
    expect(result.database.db.prepare('SELECT MAX(version) AS version FROM schema_migrations').get()).toMatchObject({ version: 21 })
    ...
    expect(fs.readdirSync(path.join(dir, 'backups')).some((name) => name.startsWith('pre-migration-v8-to-v21-'))).toBe(true)
  })
