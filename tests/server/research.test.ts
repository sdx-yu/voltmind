...
describe('R1 local research evidence and release readiness', () => {
  ...
  it('backs up v13 before migrating through v16 and creates research, cohort and wave tables', () => {
    ...
    expect(fs.readdirSync(path.join(dir, 'backups')).some((name) => name.startsWith('pre-migration-v13-to-v17-'))).toBe(true)
  })
  ...
  it('serves the authenticated journey, verifies packages and keeps public release NO-GO', async () => {
    ...
    expect(support).toMatchObject({ format: 'bbd-support-v1', manifest: { appVersion: '1.7.0', database: { schemaVersion: 17, integrity: 'ok' }, counts: { cohortParticipants: 0, researchWaves: 0 }, privacy: { containsManuscriptText: false, containsPaths: false } } })
