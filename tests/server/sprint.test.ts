...
describe('V2-F trustworthy quiet sprints and offline boards', () => {
  ...
  it('backs up v10 and applies the v11 sprint schema', () => {
    ...
    db.db.exec('DROP TABLE template_events; DROP TABLE template_applications; DROP TABLE template_grants; DROP TABLE template_package_resources; DROP TABLE template_packages; DROP TABLE template_resources; DROP TABLE sprint_board_cards; DROP TABLE sprint_boards; DROP TABLE sprint_result_cards; DROP TABLE sprint_events; DROP TABLE sprint_samples; DROP TABLE sprint_sessions; DELETE FROM schema_migrations WHERE version IN (11,12);')
    ...
    expect(fs.readdirSync(path.join(dir, 'backups')).some((name) => name.startsWith('pre-migration-v10-to-v12-'))).toBe(true)
  })
