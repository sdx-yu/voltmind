...
describe('V2-V canon-bound visual anchors and storyboards', () => {
  ...
  it('backs up v12 before migration v13 and creates the visual data model', () => {
    ...
    db.db.exec('DROP TABLE research_cohort_submissions; DROP TABLE research_cohort_participants; DROP TABLE research_cohort_deletion_receipts; DROP TABLE research_events; DROP TABLE research_tasks; DROP TABLE research_enrollments; DROP TABLE visual_events; DROP TABLE storyboard_cards; DROP TABLE storyboards; DROP TABLE visual_candidates; DROP TABLE visual_anchors; DROP TABLE visual_assets; DELETE FROM schema_migrations WHERE version IN (13,14,15);')
    ...
    expect(fs.readdirSync(path.join(dir, 'backups')).some((name) => name.startsWith('pre-migration-v12-to-v15-'))).toBe(true)
  })
