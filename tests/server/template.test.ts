...
describe('V2-P local template packages', () => {
  ...
  it('backs up v11 before migration v12 and creates content-addressed tables', () => {
    ...
    db.db.exec('DROP TABLE template_events; DROP TABLE template_applications; DROP TABLE template_grants; DROP TABLE template_package_resources; DROP TABLE template_packages; DROP TABLE template_resources; DELETE FROM schema_migrations WHERE version=12;')
    ...
    expect(fs.readdirSync(path.join(dir, 'backups')).some((name) => name.startsWith('pre-migration-v11-to-v12-'))).toBe(true)
  })
