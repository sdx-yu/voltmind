...
| 制品 | 大小 | SHA-256 |
|---|---:|---|
| `release/笔不怠_2.1.0_aarch64.app.zip` | 44,996,174 bytes | `60303c4b80696f0be2173fc3bae938d3ad2ee0d7c2053680000656dbf948a603` |
| `release/笔不怠_2.1.0_aarch64.dmg` | 44,967,493 bytes | `31cd70fe0a8629026c832b2752b78450c83d48cac4e581d649510d1c50699841` |
...
- 真实启动刚构建的 `.app` 后，sidecar 只监听随机回环端口 `127.0.0.1:59422`；
...
- 首轮总门曾发现浏览器测试只等待 Vite、未等待 API 的启动竞态；已增加最长 8 秒的有限 API 就绪重试。本次打包后完整 Vitest 复跑 167 项通过，生产构建与独立 E2E 8 条均通过；
...
| 门 | 状态 | 依据 |
|---|---|---|
| G4-SOURCE | PASS | 四处版本一致，安装包由干净提交 `058849c` 对应的已提交源码重建并入库 |
...
当前可以分发给明确知情的 Apple Silicon 内部测试者，但不能宣传为已签名、公证或公开发布版本。七项发布门当前为 3 项通过、4 项外部条件阻塞。下一步须在真实外部条件到位后执行 G4-B 工具包，补齐 Developer ID 与 Apple 公证、Windows x64 真机、移动真机和真实作者试用证据；不得继续用新增本机功能替代这些验证。
